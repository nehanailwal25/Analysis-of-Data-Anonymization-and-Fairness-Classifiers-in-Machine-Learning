...
class StructuredDataset(Dataset):
    ...
    def _de_dummy_code_df(df, sep="=", set_category=False):
        ...
        for f_name in feature_names_nodum:
            df_new[f_name] = df[f_name].values.copy()

        for f_name, vl in feature_names_dum_d.items():
            for v in vl:
                df_new.loc[df[f_name+sep+str(v)] == 1, f_name] = str(v)

        if set_category:
            for f_name in feature_names_dum_d.keys():
                df_new[f_name] = df_new[f_name].astype('category')
    ...
    def _parse_feature_names(feature_names, sep="="):
        ...
        for f_name in feature_names:
            if sep in f_name:
                f_name_dum, v = f_name.split(sep, 1)
                feature_names_dum_d[f_name_dum].append(v)
            else:
                feature_names_nodum.append(f_name)
