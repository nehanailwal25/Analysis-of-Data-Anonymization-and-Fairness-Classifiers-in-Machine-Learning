...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    Function for training the model subject to various fairness constraints.
    ...
    w = Variable(num_features) # weight vector

    # initialize a random value of w
    np.random.seed(112233)
    w.value = np.random.rand(x.shape[1])

    if cons_params is None: # to train a simple classifier, no fairness constraints
        constraints = []
    else:
    ...
    # checking if the fairness constraint is satisfied
...
#defining the distance boundary
...
#computing the covariance between sensitive attributes
