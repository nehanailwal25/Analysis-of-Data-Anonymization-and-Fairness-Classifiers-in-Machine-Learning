...
#training the model according to various fairness constraints
...
#computing the p-rule based on Doctrine of disparate impact
...
#Adding intercept to the data before linear classification
def add_intercept(x):
...
#for doing the one hot encoding
...
def get_tpr_tnr(y_true, y_pred):
    ...
    print("TPR: ", tp/(tp+fn), "TNR: ", tn/(fp+tn) )    

def get_fpr_fnr(x_control_tst, y_test, y_test_predicted):
    TP = 0
    FP = 0
    TN = 0
    FN = 0
    
    k = list(x_control_tst)[0]
    print(k)
    sensitive_key = x_control_tst.get(k)
    
    
    for i, val in enumerate(sensitive_key):
        if(val == 1):
    ...
    TPR1 = TP/(TP+FN)
    TNR1 = TN/(FP+TN)
    print("Privilleged FPR:", FP/(FP+TN), "FNR:", FN/(TP+FN))
    print("TPR:", TP/(TP+FN), "TNR:", TN/(FP+TN))
    ...
    TPR = TP/(TP+FN)
    TNR = TN/(FP+TN)
    print("Protected FPR:", FP/(FP+TN), "FNR:", FN/(TP+FN))
    print("TPR:", TP/(TP+FN), "TNR:", TN/(FP+TN))
    print("------------------------------")
    print("TPRB:", TPR1-TPR, "TNRB:", TNR1-TNR)
    ...
    print(metrics.precision_recall_fscore_support(y_test, y_test_predicted, average='binary'))
    print("DI: ", DI(sensitive_key, y_test, y_test_predicted))
    get_tpr_tnr(y_test, y_test_predicted)

    
def DI(sensitive_key, y_test, y_pred):
...
#knowing the train test accuracy of model.
def check_accuracy(model, x_train, y_train, x_test, y_test, x_control_tst, y_train_predicted, y_test_predicted):
    ...
    if model is not None:
        ...
        sensitive_key = x_control_tst.get(list(x_control_tst)[0])
        m.correctness(y_test, y_test_predicted)
        m.SPD(sensitive_key, y_test, y_test_predicted)
        m.EO_PP(sensitive_key, y_test, y_test_predicted)
    ...
    return train_score, test_score, correct_answers_train, correct_answers_test, y_test_predicted
...
#For getting the covariance between sensitive features and distance from decision boundary
...
#getting the list of constraints to be fed to the minimizer
...
def split_into_train_test(x_all, y_all, x_control_all, train_fold_size, w=None):
...
def plot_cov_thresh_vs_acc_pos_ratio(x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs):
    ...
    # vary the covariance threshold using a range of decreasing multiplicative factors and see the tradeoffs between accuracy and fairness
    it = 0.05
